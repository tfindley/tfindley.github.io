...
Data written to the QXS Trays in the Managed Storage environment is protected under profiles, which specify to back up the data every hour, or every 24 hours, to either both Lattus platforms or to both Lattus platforms and to Tape.

| Profile | Minimum Storage Time | Number of Copies | Replication Destinations   |
| ------- | ------------------- | ---------------- | -------------------------- |
...
Files larger than 128KB are split into 16 parts and stored across the lattus storage nodes. it is a 16x4 split so we can lose up to four parts and still rebuild the file.
Everything less than 128KB is copied 4 times instead of being broken down into 16

#### Versioning and Snapshots
StorNext file system does not create versions or snapshots. Storage Manager creates versions on Lattus object storage.
3.7% of data are versioned more than once (13-Feb-2019)

#### Truncation
This is enabled on Lattus protected StorNext filesystems (AP-SNFS / MP-SNFS). All files are truncation candidates once they are backed up on Lattus stacks (I.e: AP + MP + Tape).
